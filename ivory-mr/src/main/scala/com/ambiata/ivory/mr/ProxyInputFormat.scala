...
class ProxyInputFormat[A, B] extends InputFormat[SplitKey[A], B]  {
  ...
  def combine(job: Job, formatClass: Class[_ <: InputFormat[_, _]], mapperClass: Class[_ <: Mapper[_, _, _, _]]): List[InputSplit] = {
    ...
    val splits: Array[InputSplit] = format.getSplits(job).asScala.toArray
    ...
    splits.groupBy(_.getLocations.mkString("|")).values.toList.flatMap(v => {
      ...
      val combined =
        if (extra != 0)
          Partition.partitionGreedily(small.toVector, buckets, (x: InputSplit) => x.getLength)
            .map(v => new ProxyInputSplit(v.toArray, formatClass, mapperClass, configuration))
        else
          Vector()
      combined ++ ok.map(s => new ProxyInputSplit(Array(s), formatClass, mapperClass, configuration))
    })
  }
