...
case class InputSpecification(formatClass: Class[_ <: InputFormat[_, _]], mapperClass: Class[_ <: Mapper[_, _, _, _]], paths: List[Path])
...
object InputSpecification {
  ...
  def fromThrift(configuration: Configuration, inputs: InputSpecificationsThrift): List[InputSpecification] = {
    inputs.inputs.asScala.map(i => {
      ...
      val mapperClass = configuration.getClassByName(i.mapper).asInstanceOf[Class[Mapper[_, _, _, _]]]
