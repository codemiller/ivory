...
object Snapshots {
   ...
   */
  def takeIncrementalSnapshot(repository: Repository, date: Date): ResultTIO[SnapshotMetadata] =
    for {
      latest  <- SnapshotMeta.latestUpToDateSnapshot(repository, date)
      meta    <- latest match {
        case Some(m) => ResultT.safe[IO, SnapshotMetadata](m).info(s"Not running snapshot as already have a snapshot for '${date.hyphenated}' and '${m.storeId}'")
   ...
   */
  def createSnapshot(repository: Repository, date: Date): Option[SnapshotMetadata] => ResultTIO[SnapshotMetadata] = (previousSnapshot: Option[SnapshotMetadata]) =>
    for {
      ...
      _           <- runSnapshot(repository, newSnapshot, previousSnapshot, date, output).info(s"""
                                 ...
                                 | Feature Store  : ${newSnapshot.storeId.render}
   ...
   */
  def runSnapshot(repository: Repository, newSnapshot: SnapshotMetadata, previousSnapshot: Option[SnapshotMetadata], date: Date, output: ReferenceIO): ResultTIO[Unit] =
    for {
   ...
   */
  private def job(repository: Repository, previousSnapshot: Option[SnapshotMetadata],
    ...
    for {
      ...
      incrementalPath =  previousSnapshot.map(meta => repository.snapshot(meta.id).toHdfs)
  ...
  def snapshot(repoPath: Path, date: Date, incremental: Boolean, codec: Option[CompressionCodec]): ScoobiAction[Path] = for {
    ...
    snap <- ScoobiAction.fromResultTIO(takeSnapshot(repo, date, incremental).map(res => repo.snapshot(res.id).toHdfs))
  } yield snap
