...
import com.ambiata.ivory.storage.metadata.{SnapshotManifest => SM}
  ...
  val cmd = IvoryCmd.withRepo[CliArguments](parser, CliArguments(LocalDate.now(), SquashConfig.default, ExtractOutput()), {
    repo => configuration => c =>
      ...
      IvoryT.fromResultTIO { for {
        ...
        _    <- SquashJob.squashFromSnapshotWith(repo, meta, c.squash, of.outputs.map(_._2)) {
          (input, dictionary, manifest) => Extraction.extract(of, repo.toIvoryLocation(input), dictionary, ManifestFlavour.SnapshotOutput, manifest).run(r)
        }
      } yield List(
        banner,
        s"Output path: ${meta.snapshotId}",
        res.incremental.cata((incr: SM) => s"Incremental snapshot used: ${incr.snapshotId}", "No Incremental Snapshot was used."),
        "Status -- SUCCESS") }
  })
}
