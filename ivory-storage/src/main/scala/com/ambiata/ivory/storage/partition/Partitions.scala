...
object Partitions {
  def getFromFactset(repository: Repository, factset: FactsetId): ResultTIO[List[Partition]] =
    FactsetManifest.read(repository, factset).flatMap({
      case None =>
        scrapeFromFactset(repository, factset)
      case Some(manifest) =>
        manifest.partitions.pure[ResultTIO]
    })

  def scrapeFromFactset(repository: Repository, factset: FactsetId): ResultTIO[List[Partition]] =
    for {
