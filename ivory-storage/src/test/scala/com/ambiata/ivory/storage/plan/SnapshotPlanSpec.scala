...
"""
  ...
  object scenario1 {
    ...
    def exactDataset =
      prop((scenario: RepositoryScenario, id: SnapshotId, info: SnapshotInfo) => {
        val snapshot = Snapshot(id, scenario.at, scenario.commit.store, scenario.commit.dictionary.some, info)
