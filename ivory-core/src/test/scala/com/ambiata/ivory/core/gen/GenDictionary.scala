      ...
      case StructEncoding(values) =>
        val subexpGen = Gen.oneOf(values.toList).flatMap {
          case (name, sve) => Gen.oneOf(subExpression(sve.encoding).map(se => StructExpression(name, se)),
                  sve.encoding match {
                    case IntEncoding | LongEncoding | DoubleEncoding => numericSubs.map(se => Inverse(StructExpression(name, se)))
                    case _  => Gen.const(NumFlips).map(se => Inverse(StructExpression(name, se)))
                  })
        }
